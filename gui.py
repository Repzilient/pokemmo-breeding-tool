...
class PriceInputDialog(tk.Toplevel):
    ...
    """
    def __init__(self, parent, required_stats: Set[str], price_manager: PriceManager, on_confirm, relevant_egg_groups: List[str] = None):
    ...
    def _create_widgets(self):
        ...
        # Smart Label Logic
        egg_group_label_m = "EggGroup (M)"
        egg_group_label_f = "EggGroup (F)"

        if self.relevant_egg_groups:
            joined_groups = "/".join(self.relevant_egg_groups)
            egg_group_label_m = f"EggGroup ({joined_groups}) (M)"
            egg_group_label_f = f"EggGroup ({joined_groups}) (F)"
        ...
        for i, stat in enumerate(self.required_stats):
            ...
            def create_entry(category, gender, col):
                ...
                # Pre-fill logic: Use specific group if available (Option A integration)
                lookup_category = category
                if category == "EggGroup" and self.relevant_egg_groups:
                    # Try to find a price for the first relevant group
                    first_group = self.relevant_egg_groups[0]
                    spec_price = self.price_manager.get_price(stat, first_group, gender)
                    if spec_price != 999999999:
                        lookup_category = first_group

                val = self.price_manager.get_price(stat, lookup_category, gender)
                if val != 999999999:
    ...
    def _confirm(self):
        for stat, entries in self.inputs.items():
            try:
                ...
                # Save Egg Groups (Option A: Update Specific Groups + Generic)
                eg_m = get_val(entries["EggGroup_M"])
                eg_f = get_val(entries["EggGroup_F"])

                # Always save generic (fallback)
                self.price_manager.set_price(stat, "EggGroup", "M", eg_m)
                self.price_manager.set_price(stat, "EggGroup", "F", eg_f)

                # Update SPECIFIC groups if known
                if self.relevant_egg_groups:
                    for group in self.relevant_egg_groups:
                        self.price_manager.set_price(stat, group, "M", eg_m)
                        self.price_manager.set_price(stat, group, "F", eg_f)
